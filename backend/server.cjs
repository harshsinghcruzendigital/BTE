...
async function buildAnalyticsSnapshot(currentUser) {
  ...
  const snapshot = {
    metrics: {
      ...
      navigationLinks: content.site?.navigation?.length || 0,
      projectCards: content.pages?.projects?.items?.length || 0,
      heroMessages: (content.hero?.messages || []).length || (content.hero?.title ? 1 : 0),
